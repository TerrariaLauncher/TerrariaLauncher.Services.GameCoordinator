...
{
    class NetModuleSerializer<TPayload> : StructureSerializer<NetModule<TPayload>> where TPayload : class, IModuleStructure
    {
        ...
        protected override async Task Implementation(NetModule<TPayload> structure, PipeWriter pipeWriter, CancellationToken cancellationToken = default)
        {
            ...
            await this.payloadSerializer.Serialize(structure.Payload, pipeWriter, false, cancellationToken).ConfigureAwait(false);
        }
    }
}
